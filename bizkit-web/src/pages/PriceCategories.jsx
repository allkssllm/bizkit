...
const PriceCategories = () => {
    ...
    const fetchCategories = async () => {
        try {
            ...
            alert('Gagal mengambil data kategori harga');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            await api.delete(`/master/price-categories/${deleteModal.id}`);
            fetchCategories();
            setDeleteModal({ isOpen: false, id: null });
        } catch (error) {
            console.error('Failed to delete price category:', error);
            alert(`Gagal menghapus kategori harga: ${error.response?.data?.error || 'Sedang digunakan'}`);
            setDeleteModal({ isOpen: false, id: null });
        }
    };
        ...
        <div className="flex flex-col h-full bg-gray-50">
            ...
            <div className="flex-1 p-6 relative">
                ...
                <div className="bg-white border border-gray-200 rounded-lg overflow-x-auto">
                    <table className="min-w-full divide-y border-collapse min-w-[600px]">
                        <thead className="bg-[#cbd5e1] text-[#334155]">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3 text-left text-xs font-bold tracking-wider">
                                    <div className="flex justify-between items-center">
                                        Nama Kategori Harga Tambahan <span className="text-[10px] text-gray-500 ml-1">▲▼</span>
                                    </div>
                                </th>
                                <th scope="col" className="px-6 py-3 text-center text-xs font-bold tracking-wider w-32 border-l border-gray-300">
                                    <div className="flex justify-center items-center">
                                        Aksi <span className="text-[10px] text-gray-500 ml-1">▲▼</span>
                                    </div>
                                </th>
                            </tr>
                        </thead>
                                ...
                                filteredCategories.map((cat, index) => (
                                    <tr key={cat.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium border-l border-gray-200">
                                            <button
                                                onClick={() => navigate(`/price-category/form/${cat.id}`)}
                ...
                {/* Floating Add Button */}
                <button
                    onClick={() => navigate('/price-category/form')}
