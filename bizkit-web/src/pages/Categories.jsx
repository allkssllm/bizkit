...
const Categories = () => {
    ...
    const handleDelete = async () => {
        ...
        try {
            await api.delete(`/master/categories/${deleteModal.id}`);
            fetchCategories();
            setDeleteModal({ isOpen: false, id: null });
        } catch (error) {
            console.error('Failed to delete category:', error);
            alert(`Gagal menghapus kategori: ${error.response?.data?.error || 'Sedang digunakan'}`);
            setDeleteModal({ isOpen: false, id: null });
        }
    };
                                ...
                                filteredCategories.map((cat, index) => (
                                    <tr key={cat.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium border-l border-gray-200">
                                            <button
                                                onClick={() => navigate(`/category/form/${cat.id}`)}
