...
const CategoryFormPage = () => {
    ...
    const fetchCategory = async () => {
        try {
            ...
            setErrorModal({ isOpen: true, message: 'Gagal mengambil data kategori.' });
        }
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!name.trim()) {
            setErrorModal({ isOpen: true, message: 'Nama Kategori wajib diisi' });
            return;
        }
        ...
        try {
            if (isEdit) {
                await api.put(`/master/categories/${id}`, { name });
            } else {
                await api.post('/master/categories', { name });
            }
            navigate('/categories');
        } catch (error) {
            console.error('Failed to save category:', error);
            setErrorModal({ isOpen: true, message: `Gagal menyimpan kategori: ${error.response?.data?.error || error.message}` });
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="flex-1 p-6">
                <div className="bg-white border border-gray-200 shadow-sm flex flex-col min-h-[300px]">
                    <div className="p-8 flex-1">
                        <form id="category-form" onSubmit={handleSubmit} className="space-y-6 max-w-full">
                            <div>
