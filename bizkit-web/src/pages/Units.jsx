...
const Units = () => {
    ...
    const handleDelete = async () => {
        ...
        try {
            await api.delete(`/master/units/${deleteModal.id}`);
            fetchUnits();
            setDeleteModal({ isOpen: false, id: null });
        } catch (error) {
            console.error('Failed to delete unit:', error);
            alert(`Gagal menghapus satuan: ${error.response?.data?.error || 'Sedang digunakan'}`);
            setDeleteModal({ isOpen: false, id: null });
        }
    };
                        ...
                        <thead className="bg-[#cbd5e1] text-[#334155]">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3 text-left text-xs font-bold tracking-wider">
                                    <div className="flex justify-between items-center">
                                        Satuan <span className="text-[10px] text-gray-500 ml-1">▲▼</span>
                                    </div>
                                </th>
                                ...
                                filteredUnits.map((unit, index) => (
                                    <tr key={unit.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium border-l border-gray-200">
                                            <button
                                                onClick={() => navigate(`/unit/form/${unit.id}`)}
