...
const PaymentMethodPage = () => {
    ...
    const fetchMethods = async () => {
        ...
        try {
            ...
            const response = await axios.get('http://localhost:8081/api/master/payment-methods', {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:8081/api/master/payment-methods/${deleteTarget}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchMethods();
        } catch (err) {
